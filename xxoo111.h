...
// هنا بملى البورد بتعتي بقيمة ابتدائية صفر
template<typename T>
Pyramic_X_O_Board<T>::Pyramic_X_O_Board() {
  this->rows = 3;
  this->columns = 5; // Maximum columns
...
//           this->board[i][0] != 0) ||
//          (this->board[0][j] == this->board[1][j] && this->board[1][j] == this->board[2][j] &&
//           this->board[0][j] != 0)) {
//        return true;
//      }
//    }
...
Pyramic_X_O_Random_Player<T>::Pyramic_X_O_Random_Player(T symbol) : RandomPlayer<T>(symbol) {
  this->dimension = 5; // Adjusted for maximum dimension
